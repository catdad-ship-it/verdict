    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ minWidth: 0 }}>
          <p style={{ fontSize: 11, color: 'rgba(192,120,24,0.6)', letterSpacing: 3, margin: '0 0 4px' }}>VERDICT · SHARED LIST</p>
          ...
          <p style={{ fontSize: 11, color: 'var(--cream-dim)', margin: '4px 0 0', letterSpacing: 1 }}>
        ...
        <Link
          ...
          style={{
            ...
            fontSize: 11, color: 'var(--amber)', letterSpacing: 2,
                ...
                <div key={item.id} style={{
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                      {item.release_year && <span style={{ fontSize: 11, color: 'var(--cream-dim)' }}>{item.release_year}</span>}
                      {item.runtime && <span style={{ fontSize: 11, color: 'var(--cream-dim)' }}>{Math.floor(item.runtime / 60)}h {item.runtime % 60}m</span>}
                      {item.media_type === 'tv' && (
                        <span style={{ fontSize: 11, color: '#C0E8AC', background: 'rgba(34,80,34,0.4)', padding: '1px 5px', borderRadius: 2, letterSpacing: 1 }}>SHOW</span>
                      )}
                    </div>
                    {(item.imdb_rating || item.rt_score) && (
                      <div style={{ display: 'flex', gap: 6, marginTop: 5 }}>
                        {item.imdb_rating && <span className="rating-imdb"><b className="wordmark">IMDb</b>{item.imdb_rating}</span>}
                        {item.rt_score && <span className="rating-rt"><b className="wordmark">RT</b>{item.rt_score}%</span>}
                      </div>
                    )}
                  </div>
                </div>
